 ...
 *
 * Example:
 * [cursorTarget] call kat_10thMods_faction_fnc_addPlaneMenu;
 *
 * Public: Yes
 */
...
private _guiArray = _drones + _helicopters + _planes + [localize ELSTRING(faction_generic,other), [localize ELSTRING(faction_generic,remove), "REMOVE"]];

// vehicle GUI
[_object, 0, [],
    [QGVAR(planeActionParent), localize ELSTRING(faction_generic,vehicle), "", {}, {true}, {}, [], _pos] call ace_interact_menu_fnc_createAction
...

{
    if (_x isEqualType "") then {
        ...
        [_object, 0, _currentParent,
            [QGVAR(planeAction) + (str _forEachIndex), _x select 0, "", EFUNC(faction_generic,createVehicle), {true}, {}, [_x select 1, true]] call ace_interact_menu_fnc_createAction
        ] call ace_interact_menu_fnc_addActionToObject;
    };
} forEach _guiArray;
