...
if (!("KAT_artilleryTable" in (ace_player call ace_common_fnc_uniqueItems))) exitWith {};
    ...
    if ((_showRangetable == 1) && {!(_x in _vehicleAdded)}) then {
        ...
        if (_index != -1) then {
            ...
            private _condition = {
                ...
                ("KAT_artilleryTable" in (_player call ace_common_fnc_uniqueItems)) && {[_player, objNull, ["notOnMap", "isNotSitting", "isNotInside"]] call ace_common_fnc_canInteractWith}
            };
            private _displayName = format ["%1%2", getText (_vehicleCfg >> "displayName"),["","*"] select _advCorrection];
            private _action = [format ['QGVAR(%1)',_index], _displayName, QPATHTOF(UI\icon_rangeTable.paa), _statement, _condition, {}, _info] call ace_interact_menu_fnc_createAction;
            [ace_player, 1, ["ACE_SelfActions", "ACE_Equipment"], _action] call ace_interact_menu_fnc_addActionToObject;
